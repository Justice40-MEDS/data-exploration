...
```{r}
# Create static map
ggplot(contiguous_data) +
  geom_sf(aes(fill = standardized_g_star)) +
  scale_fill_viridis_c(name = "Standardized G*") +
  theme_minimal() +
  labs(title = "Standardized G* Values - Contiguous US") +
  theme(
...
```{r}
library(mapview)
mapview(contiguous_data, zcol = "standardized_g_star")
```
